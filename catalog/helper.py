...
def get_company():
    """
    Get the company name from the Settings in the database
    """
    company, is_created = Setting.objects.get_or_create(key="company")
    if is_created:
        company.value = "JKF-VICTORY LIMITED"
        company.save()
...
def get_can_negative_stock():
    """
    Get from database if negative stock is allowed. Returns True when allowed.
    """
...
def setup_handling(request):
    """
    Setting that gets the Cash, current order and currency
    """
...
def get_current_user_order(username):
    ...
    q = Order.objects.filter(user=usr, done=False).order_by('-last_change')
    if q.count() >= 1:
        return q[0]
    else:
...
def order_item_from_product(product, order):
    """
    Creates an Order-Item from a given Product, to be 
    added to an Order
    """
    return Order_Item.objects.create(product=product, order=order, price=product.price, quantity=product.quantity, name=product.name)


def product_list_from_order(order):
    """
    Return a list of Products that appear in an Order
    """
    product_list = []
