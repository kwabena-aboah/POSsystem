$(document).ready(function() {
 ...
 var app = new Vue({
     ...
     methods: {
         // setToEditing: function(item){
         //    this.isEditingID = item.id;
         //    setTimeout(()=> {
         //      document.getElementById(`tag-edit-${item.id}`).focus()
         //    }, 1)
         // },
         ...
         apdateProduct: function(product_id) {
            this.isEditing = true;
             fetch("/api/orders/current/items/patch/" + encodeURIComponent(product_id) + "/", {method: 'PATCH'})
                .then((response) => {
         ...
         searchProduct: function(product_id) {
             fetch("/api/orders/current/items/search/" + encodeURIComponent(product_id) + "/", {method: 'PUT'})
                 .then(response => {
                     if (response.status == 404) {
                         window.alert("Product not found");
                     } else if (response.status != 200) {
         ...
         scanCode: function() {
             this.searchProduct(document.getElementById('codeInput').value);
